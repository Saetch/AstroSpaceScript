...
function OrbitingPlanet({
    ...
    useFrame((state, delta) => {
        ...
        if (positionGroup.current) {
            const cameraDistance = state.camera.position.distanceTo(positionGroup.current.getWorldPosition(worldPosition.current))
            const hitRadius = THREE.MathUtils.clamp(cameraDistance * 0.018, planet.radius * 1.8, 14)
            const glowRadius = THREE.MathUtils.clamp(cameraDistance * 0.006, planet.radius * 1.35, 5.5)
